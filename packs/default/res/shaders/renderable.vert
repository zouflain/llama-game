...
void main(){
    ...
    for(int i=0; i<4; i++){
        mat4 transform = bones[v_data[gl_VertexID].bone_ids[i]]*v_data[gl_VertexID].bone_weights[i];
        world_pos += transform * vec4(v_pos, 1.0);
        world_norm += transform * vec4(frag_normal, 0);
    }
    world_pos = model * world_pos;
    frag_pos = vec3(world_pos);
    frag_normal = normalize(vec3(world_norm));
